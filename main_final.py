...
class Automat:
    ...
    def __init__(self, file_name):
        ...
        self.nodes = {i for i in file.readline().split()} #nodurile
        self.sigma = {i for i in file.readline().split()} #limbajul
        print(self.sigma)
        self.init_node = file.readline()[0]
    ...
    def drawGraph(self):
        ...
        for ni, char in self.muchie: #daca tulplul e in dictionarul muchie
            nf = self.muchie[(ni, char)]
            if (char, ni, nf) in self.path:
                G.add_edge(ni, nf, color=clr) #adaug muchii colorate corespunzator
            else:
                G.add_edge(ni, nf)
